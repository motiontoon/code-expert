...
export default function Register() {
  ...
  const { register: doRegister, connectGitHub } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Account created! Welcome to Codex Forge.');
      navigate('/dashboard');
    } catch (err: unknown) {
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-3">
            <div className="w-12 h-12 bg-forge-600 rounded-xl flex items-center justify-center text-white font-bold text-lg">
              CF
            </div>
          </Link>
          <h1 className="text-2xl font-bold text-white mt-4">Create Account</h1>
          <p className="text-dark-muted mt-1">Start coding with AI automation</p>
        </div>

        <button
          onClick={connectGitHub}
          className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-dark-surface border border-dark-border rounded-lg text-white font-medium hover:bg-dark-border transition-colors mb-6"
        >
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-4 bg-dark-bg text-dark-muted">or create with email</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="input" placeholder="johndoe" required minLength={3} />
          </div>
          <div>
        ...
        <p className="text-center text-sm text-dark-muted mt-6">
          ...
          <Link to="/login" className="text-forge-400 hover:text-forge-300">Sign in</Link>
        </p>
      </div>
    </div>
  );
